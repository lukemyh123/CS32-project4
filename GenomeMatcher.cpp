...
private:
	...
	void findRelatedGenomesHelper(vector<GenomeMatch>& results, GenomeMatch new_GenomeMatch, double matchPercentThreshold) const;
};
...
void GenomeMatcherImpl::findRelatedGenomesHelper(vector<GenomeMatch>& results, GenomeMatch new_GenomeMatch, double matchPercentThreshold) const
{
	if (results.size() == 0)
	{
		if (new_GenomeMatch.percentMatch >= matchPercentThreshold / 100)
			results.push_back(new_GenomeMatch);
	}

	if (results.size() != 0)
	{
		for (vector<GenomeMatch>::iterator it = results.begin(); it != results.end(); ++it)
		{
